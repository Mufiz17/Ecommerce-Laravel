                    ...
                    <div class="cart-table__wrapper">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="qty-control position-relative">
                                                ...
                                                <form action="{{ route('cart.decrease.qty', ['rowId' => $item->rowId]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="qty-control__reduce">-</div>
                                                </form>
                                                <form action="{{ route('cart.increase.qty', ['rowId' => $item->rowId]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                        ...
                                        <td>
                                            <form action="{{ route('cart.delete.qty', ['rowId' => $item->rowId]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <div class="cart-table-footer">
                            @if (!Session::has('discount'))
                                <form action="{{ route('cart.disc.add') }}" method="POST"
                            ...
                            @else
                                <form action="{{ route('cart.disc.delete') }}" method="POST"
                                    class="position-relative bg-body">
                                    @csrf
                                    <input class="form-control" type="text" name="discount_code"
                                        placeholder="Discount Code"
                                        value="@if (Session::has('discount')) {{ Session::get('discount')['code'] }} Applied! @endif">
                                    <input class="btn-link fw-medium position-absolute top-0 end-0 h-100 px-4"
                                        type="submit" value="REMOVE DISCOUNT">
                                </form>
                            @endif
                                ...
                                @if (Session::has('discounts'))
                                    <table class="cart-totals">
                                        <tbody>
                                            ...
                                            <tr>
                                                <th>Discount {{ Session::get('discount')['code'] ?? '' }}</th>
